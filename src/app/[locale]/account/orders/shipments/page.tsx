...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: t("orders.shipmentsLabel") + " — Leaf & Cup",
  };
}
...
export default async function ShipmentsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const orders = await getOrdersForUser(session.user.id, ["SHIPPED", "DELIVERED"]);
  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-12 md:py-16">
      <nav className="mb-8 flex items-center gap-1.5 text-sm text-muted-foreground">
        ...
        <ChevronRight className="h-3.5 w-3.5" />
        <span className="text-foreground">{t("orders.shipmentsLabel")}</span>
      </nav>

      <h1 className="mb-8 font-heading text-2xl font-medium tracking-tight md:text-3xl">
        {t("orders.shipmentsLabel")}
      </h1>

      <OrderList
        orders={orders}
        emptyText={t("orders.shipmentsEmpty")}
        showShippingAddress
        locale={locale}
        t={t}
        dict={{

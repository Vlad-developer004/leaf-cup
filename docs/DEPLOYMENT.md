...
| **GitHub Actions** | CI (lint/typecheck/build) on every push/PR, plus a Playwright e2e job that runs a real Stripe test-mode payment against a throwaway Postgres service container | github.com, `.github/workflows/ci.yml` + `.github/workflows/e2e.yml` — the e2e job needs `STRIPE_SECRET_KEY` and `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` (test mode) added as repo secrets under Settings → Secrets and variables → Actions, or the payment step fails (everything up to the checkout page still runs) |
...
| **Rotate any secret** (DB password, Stripe keys, `AUTH_SECRET`, Google client secret) | Update it in Vercel → Settings → Environment Variables, then**Redeploy** — env var changes don't apply to an already-running deployment. If you rotated `STRIPE_SECRET_KEY`/`NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`, also update the same-named secrets in GitHub → Settings → Secrets and variables → Actions, or the e2e workflow's payment step starts failing.                                                                                                                                                                                |
